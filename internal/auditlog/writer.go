...
func scoreBreakdown(result pipeline.Result) string {
	...
	}{TotalScore: detectionScore(result), Threshold: result.ScoreThreshold, Rules: make([]ruleScore, 0, len(result.Detection.Matches)+len(result.Semantic.Matches))}
	for _, match := range result.Detection.Matches {
...
func attackType(result pipeline.Result) string {
	...
	if len(result.Semantic.Matches) > 0 {
		return defaultString(primarySemanticMatch(result).Group, "semantic")
	}
...
func firstRuleID(result pipeline.Result) string {
	...
	if len(result.Semantic.Matches) > 0 {
		return fmt.Sprintf("%d", primarySemanticMatch(result).ID)
	}
	return ""
}
...
func firstRuleMessage(result pipeline.Result) string {
	...
	if len(result.Semantic.Matches) > 0 {
		match := primarySemanticMatch(result)
		if strings.TrimSpace(match.Message) != "" {
...
func primarySemanticMatch(result pipeline.Result) detection.MatchedRule {
	if len(result.Semantic.Matches) == 0 {
		return detection.MatchedRule{}
	}
	for _, match := range result.Semantic.Matches {
		if !strings.EqualFold(match.Group, "semantic-entropy") {
			return match
		}
	}
	return result.Semantic.Matches[0]
}
