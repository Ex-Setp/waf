...
func (s *Server) reloadRuntime(r *http.Request) {
	if s.runtime != nil {
		_ = s.runtime.Reload(r.Context())
	}
	_ = s.reloadPolicies(r.Context())
}
...
func normalizePolicyMode(value string) string {
	...
	case database.PolicyModeLoose:
		return database.PolicyModeLoose
	case database.PolicyModeStrict:
		return database.PolicyModeStrict
	default:
