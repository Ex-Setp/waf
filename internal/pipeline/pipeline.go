...
func (p *Pipeline) Process(ctx context.Context, req Request) (result Result, err error) {
	...
	if p.semantic != nil && shouldRunSemantic(req, result.Detection, result.ScoreThreshold) {
		...
		if semanticResult.Decision == detection.DecisionBlock {
			threshold := semanticBlockThreshold(req)
			if semanticResult.Score >= threshold {
				result.Decision = DecisionBlock
				result.FinalAction = DecisionBlock
				result.Reason = fmt.Sprintf("semantic score %d reached threshold %d", semanticResult.Score, threshold)
				result.BlockedByStage = StageSemantic
			} else {
				result.Decision = DecisionObserve
				result.FinalAction = DecisionObserve
				result.Reason = fmt.Sprintf("semantic observed score %d below threshold %d", semanticResult.Score, threshold)
			}
		}
	}
...
func semanticBlockThreshold(req Request) int {
	switch strings.ToLower(strings.TrimSpace(req.PolicyMode)) {
	case "strict":
		return 6
	case "standard", "":
		return 8
	case "custom":
		if req.BlockScoreThreshold > 0 && req.BlockScoreThreshold < 8 {
			return req.BlockScoreThreshold
		}
		return 8
	default:
...
func shouldRunSemantic(req Request, detectionResult detection.Result, threshold int) bool {
	if req.SemanticPolicySet && !req.SemanticProtection {
		return false
	}
	if strings.EqualFold(strings.TrimSpace(req.PolicyMode), "loose") {
		return false
	}
...
func containsHighRiskToken(value string) bool {
	text := strings.ToLower(normalizer.NormalizeValue(value))
